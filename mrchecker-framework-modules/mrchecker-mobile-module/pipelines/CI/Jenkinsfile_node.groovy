node(){
	...
	properties([
		parameters([
			string(defaultValue: 'mrchecker-framework-modules/mrchecker-mobile-module/', description: 'Execute job for given Module. mobile mrchecker-framework-modules/mrchecker-mobile-module/ ', name: 'APP_WORKSPACE'),
...
MORE information here http://maven.apache.org/surefire/maven-surefire-plugin/mobiles/single-test.html''', name: 'TEST_NAME'), 
...
https://oss.sonatype.org/content/repositories/snapshots/com/capgemini/ntc/''', name: 'IS_TO_DEPLOY_REMOTE_NEXUS'), 
			...
			string(defaultValue: '', description: 'Optional list of mvn parameters, mobile -DskipTests=true -Dtest=*', name: 'MVN_PARAMETERS')
			]), 
			pipelineTriggers([])
		]);
